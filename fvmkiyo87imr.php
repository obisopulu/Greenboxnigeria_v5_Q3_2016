...
if ($pn == 1) {
    ...
    $centerPages .= '&nbsp; <a href="fvmkiyo87imr.php?pn=' . $add1 . '">' . $add1 . '</a> &nbsp;';
} else if ($pn == $lastPage) {
    $centerPages .= '&nbsp; <a href="fvmkiyo87imr.php?pn=' . $sub1 . '">' . $sub1 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <span class="pagNumActive">' . $pn . '</span> &nbsp;';
} else if ($pn > 2 && $pn < ($lastPage - 1)) {
    $centerPages .= '&nbsp; <a href="fvmkiyo87imr.php?pn=' . $sub2 . '">' . $sub2 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <a href="fvmkiyo87imr.php?pn=' . $sub1 . '">' . $sub1 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <span class="pagNumActive">' . $pn . '</span> &nbsp;';
    $centerPages .= '&nbsp; <a href="fvmkiyo87imr.php?pn=' . $add1 . '">' . $add1 . '</a> &nbsp;';
    $centerPages .= '&nbsp; <a href="fvmkiyo87imr.php?pn=' . $add2 . '">' . $add2 . '</a> &nbsp;';
...
//    $paginationDisplay .= 'Page <strong>' . $pn . '</strong> of ' . $lastPage. '&nbsp;  &nbsp;  &nbsp; ';
    ...
    if ($pn != 1) {
        ...
        $previous = $pn - 1;
        $paginationDisplay .=  "<td align='left' width='50px'>".'<a href="fvmkiyo87imr.php?pn=' . $previous . '"> Back</a> '.'</td>';
    ...
    if ($pn != $lastPage) {
		echo"";
        $nextPage = $pn + 1;
        $paginationDisplay .=  "<td align='right' width='50px'>".'<a href="fvmkiyo87imr.php?pn=' . $nextPage . '"> Next</a> '.'</td>';
...
<form action='lvrgtj4vu58un.php' method='GET'>
        ...
        <input type="hidden" name="source" value="labels"/>
        </form></div>
</td></tr></table>
<!--            Head            -->
</td></tr><tr><td style="padding-left:23px">
<form action="ekiy87t4wfszz.php" method="get">
<?php 
while($row = mysql_fetch_array($sql2))
	{
		extract($row);
		echo "<table style='float:left; margin:7px 15px'><tr><td><input type='submit' name='label' value='$labelName' style='background-image:url(labelsimg/$labelPic); font-size:0px; background-size:cover; background-position:center; background-repeat:no-repeat; border:none; height:155px; width:155px; cursor: pointer; background-color:#262626; border-radius:0px'></button>
		</td></tr><tr><td align='center'>
		<span class='thirdtextWyt'>"; 
		echo substr_replace($labelName,'',26);
...
window.onload = function() {
  	top.document.getElementById('title1').innerHTML='Labels';
	top.document.getElementById('description').content='Nigerian Record labels';
	top.document.getElementById('keywords').content='Nigerian, record, labes';
};
</script>
</body>
</html>